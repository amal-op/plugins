...
class JobController extends StorefrontController
{
    ...
    /**
     * The loader responsible for handling job page operations.
     *
     * @var JobPageLoader
     */
    private JobPageLoader $jobPageLoader;
     ...
     */
    public function __construct(JobPageLoader $jobPageLoader)
    {
        $this->jobPageLoader = $jobPageLoader;
    }
     ...
     *
     * @Route("/job/{id}", name="frontend.detail.job", methods={"GET"}, defaults={"csrf_protected"=false, "XmlHttpRequest"=true})
     *
    ...
    public function jobDetail(Request $request, SalesChannelContext $context): Response
    {
        ...
        $job = $jobPage->getJob();
        if ($job) {
            return $this->renderStorefront('@Storefront/storefront/page/job/index.html.twig', ['page' => $jobPage]);
        }
        return $this->renderStorefront('@Storefront/storefront/page/error/error-404.html.twig', ['page' => $jobPage]);
    }
}