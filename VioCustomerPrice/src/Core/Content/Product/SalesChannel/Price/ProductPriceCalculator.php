...
class ProductPriceCalculator extends AbstractProductPriceCalculator implements ResetInterface
{
    ...
    public function calculate(iterable $products, SalesChannelContext $context): void
    {
        ...
        if($customer instanceof CustomerEntity) {
            ...
            $ifDiscountIsSetUseCustomerPriceAsListPrice = $this->configService->getBool('VioCustomerPrice.config.ifDiscountIsSetUseCustomerPriceAsListPrice');
            $ignoreCalculatedPrices = $this->configService->getBool('VioCustomerPrice.config.ignoreCalculatedPrices');
            ...
            foreach ($products as $product) {
                ...
                if(!$ignoreCalculatedPrices) {
                    ...
                    $originalListPrice = $originalCalculatedPrice->getListPrice() ? $originalCalculatedPrice->getListPrice()->getPrice() : null;
                }
                else {
                ...
                if($customerPricesCollection instanceof CustomerPriceCollection && $customerPricesCollection->first() !== null) {
                    ...
                    if(
                        !$this->configService->getBool('VioCustomerPrice.config.deactivateSpecialCaseForOnlyOnePriceWithDiscount')
                        ...
                        foreach ($originalCalculatedPrices as $price) {
                            $this->setPriceOnCalculatedPrice($price, $discountPrice, $listPrice);
                        }

                        if($originalCalculatedPrices->first() !== $originalCalculatedPrice) {
                            $this->setPriceOnCalculatedPrice($originalCalculatedPrice, $discountPrice, $listPrice);
                        }

                        if($product->getCalculatedCheapestPrice()) {
                            $this->setPriceOnCalculatedPrice($product->getCalculatedCheapestPrice(), $discountPrice, $listPrice);
                        }
                    ...
                    foreach ($customerPricesCollection as $customerPrice) {
                        ...
                        $price = new QuantityPriceDefinition(
                            $this->calcCustomerPrice($customerPrice, $originalCalculatedPrice->getUnitPrice()),
                            $taxRules,
                            $curQuantity
                        );
                    ...
                    $quantityPrice = new QuantityPriceDefinition(
                        $this->calcCustomerPrice($quantityCustomerPrice, $originalCalculatedPrice->getUnitPrice()),
                        $taxRules
                    );
    ...
    {
        if($this->coreService instanceof ResetInterface) {
    ...
    {
        $applyDiscountOnCustomerPriceItSelf = $this->configService->getBool('VioCustomerPrice.config.applyDiscountOnCustomerPriceItSelf');
        if($applyDiscountOnCustomerPriceItSelf
            && $customerPrice->getDiscount() !== null
            && $customerPrice->getDiscount() !== 0.0
            && $customerPrice->getPrice() > 0
        ) {
            return $customerPrice->getPrice() * (1 - ($customerPrice->getDiscount() / 100));
        }
    ...
    protected function setPriceOnCalculatedPrice(CalculatedPrice $price, CustomerPriceEntity $discountPrice, ?float $originalListPrice): void
    {
